...
import mortiseGuiExample from "../examples/extensions/mortise-gui.ts";
import type { ExtensionAPI, ExtensionUIContext } from "../src/index.ts";

describe("Mortise GUI validation example", () => {
	it("publishes a runnable contract and restores scenario state on teardown", async () => {
		...
		expect(initial.scenarios?.[0]?.teardownCommand).toBe("mortise-gui-example-scenario-reset");
		expect(initial.snapshot?.state?.count).toBe(0);

		await handlers.get("mortise-gui-example-scenario-count")?.(JSON.stringify({ count: 7 }), { ui });
		expect((definitions.at(-1) as { snapshot?: { state?: { count?: number } } }).snapshot?.state?.count).toBe(7);
		await handlers.get("mortise-gui-example-scenario-reset")?.("{}", { ui });
