...
describe("createAgentSession OpenRouter attribution headers", () => {
	...
	it("adds default attribution headers for OpenRouter models", async () => {
		...
		expect(headers?.["HTTP-Referer"]).toBe("https://github.com/hrhgit/mortise");
		expect(headers?.["X-OpenRouter-Title"]).toBe("Mortise");
		expect(headers?.["X-OpenRouter-Categories"]).toBe("desktop-agent");
	});
	...
	it("adds OpenCode session headers", async () => {
		...
		expect(headers?.["x-opencode-client"]).toBe("mortise");
	});
