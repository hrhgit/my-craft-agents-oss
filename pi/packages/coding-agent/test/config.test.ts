...
function createNpmPrefixInstall(template = "pi-prefix-"): { prefix: string; packageDir: string } {
	...
	const scopeDir = join(root, "@mortise");
...
describe("detectInstallMethod", () => {
	...
	test("self-updates npm installs from custom prefixes", () => {
		...
		const command = getSelfUpdateCommand("@mortise/pi-coding-agent");
	...
	test("self-update respects configured npmCommand", () => {
		...
		const command = getSelfUpdateCommand("@mortise/pi-coding-agent", ["npm", "--prefix", prefix]);
	...
	test("self-updates bun global installs from bun pm bin", () => {
		...
		expect(command).toEqual({
			command: "bun",
			args: ["install", "-g", "--ignore-scripts", "--minimum-release-age=0", "@mortise/pi-coding-agent"],
			display: "bun install -g --ignore-scripts --minimum-release-age=0 @mortise/pi-coding-agent",
		});
	});
	...
	test("self-updates pnpm v11 global installs resolved through the store", () => {
		...
		const packageName = "@mortise/pi-coding-agent";
		const globalPackageDir = join(root, "11e9a", "node_modules", "@mortise", "pi-coding-agent");
	...
	test("does not self-update when npm install path is not writable", () => {
		...
		expect(getSelfUpdateCommand("@mortise/pi-coding-agent")).toBeUndefined();
		expect(getSelfUpdateUnavailableInstruction("@mortise/pi-coding-agent")).toContain(
