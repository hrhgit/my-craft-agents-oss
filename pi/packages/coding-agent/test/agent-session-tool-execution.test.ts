...
import type { ToolExecutionRequest, ToolResultRequest } from "../src/core/agent-session.ts";
...
describe("AgentSession tool execution handler", () => {
	let tempDir: string;
	let agentDir: string;

	beforeEach(() => {
		tempDir = join(tmpdir(), `pi-tool-execution-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	it("blocks tool execution when handler returns block", async () => {
		...
		await session.bindExtensions({
			toolExecutionHandler: async () => ({ action: "block", reason: "denied by host" }),
		});
