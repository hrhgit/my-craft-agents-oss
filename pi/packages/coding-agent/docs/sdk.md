...
```bash
npm install @mortise/pi-coding-agent
```
...
```typescript
import { createAgentSession, SessionManager } from "@mortise/pi-coding-agent";
...
The `Agent` class (from `@mortise/pi-agent-core`) handles the core LLM interaction. Access it via `session.agent`.
...
import { AuthStorage, ModelRegistry } from "@mortise/pi-coding-agent";
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "@mortise/pi-coding-agent";
...
```typescript
import { createAgentSession } from "@mortise/pi-coding-agent";
...
import { createAgentSession, defineTool } from "@mortise/pi-coding-agent";
...
Embedded Mortise hosts should select the Mortise extension target so Pi-only extensions are filtered before loading:
...
```typescript
import { createEventBus, DefaultResourceLoader } from "@mortise/pi-coding-agent";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@mortise/pi-coding-agent";
