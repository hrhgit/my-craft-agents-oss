...
import { type HostModelDefaultSlot, type ModelDefaultThinkingLevel, SettingsManager } from "./settings-manager.ts";

export const CURRENT_SESSION_MODEL_REFERENCE = "current-session" as const;
export type ModelReference = typeof CURRENT_SESSION_MODEL_REFERENCE | `default:${number}` | `model:${string}/${string}`;
...
export function resolveModelReference(
	...
	options: {
		...
		currentThinkingLevel?: ModelDefaultThinkingLevel;
	...
	if (defaultMatch) {
		...
		const configured = SettingsManager.create(
			options.cwd ?? process.cwd(),
			options.agentDir ?? getAgentDir(),
		).getModelDefaultSlot(slot);
		if (!configured || (options.modelRegistry && !options.modelRegistry.find(configured.provider, configured.model)))
			return undefined;
