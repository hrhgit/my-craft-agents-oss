...
Stateful agent with tool execution and event streaming. Built on `@mortise/pi-ai`.

## Installation

```bash
npm install @mortise/pi-agent-core
```
...
```typescript
declare module "@mortise/pi-agent-core" {
...
```typescript
import { Agent, streamProxy } from "@mortise/pi-agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@mortise/pi-agent-core";
