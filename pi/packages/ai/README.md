...
```bash
npm install @mortise/pi-ai
```

TypeBox exports are re-exported from `@mortise/pi-ai`: `Type`, `Static`, and `TSchema`.

## Quick Start

```typescript
import { Type, getModel, stream, complete, Context, Tool, StringEnum } from '@mortise/pi-ai';
...
```typescript
import { Type, Tool, StringEnum } from '@mortise/pi-ai';
...
```typescript
import { stream, validateToolCall, Tool } from '@mortise/pi-ai';
...
```typescript
import { readFileSync } from 'fs';
import { getModel, complete } from '@mortise/pi-ai';
...
```typescript
import { getModel, streamSimple, completeSimple } from '@mortise/pi-ai';
...
```typescript
import { getModel, stream } from '@mortise/pi-ai';
...
```typescript
import { getProviders, getModels, getModel } from '@mortise/pi-ai';
...
```typescript
import { Model, stream } from '@mortise/pi-ai';
...
```typescript
import { streamAnthropic, type AnthropicOptions } from '@mortise/pi-ai';
...
```typescript
import { getModel, complete, Context } from '@mortise/pi-ai';
...
```typescript
import { Context, getModel, complete } from '@mortise/pi-ai';
...
- OAuth login flows are not supported in browser environments. Use the `@mortise/pi-ai/oauth` entry point in Node.js.
...
```typescript
import { getEnvApiKey } from '@mortise/pi-ai';
...
```bash
npx @mortise/pi-ai login              # interactive provider selection
npx @mortise/pi-ai login anthropic    # login to specific provider
npx @mortise/pi-ai list               # list available providers
```
...
The library provides login and token refresh functions via the `@mortise/pi-ai/oauth` entry point. Credential storage is the caller's responsibility.
...
} from '@mortise/pi-ai/oauth';
```

### Login Flow Example

```typescript
import { loginGitHubCopilot } from '@mortise/pi-ai/oauth';
...
```typescript
import { getModel, complete } from '@mortise/pi-ai';
import { getOAuthApiKey } from '@mortise/pi-ai/oauth';
...
- Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@mortise/pi-ai`
