...
const TEST_MODE_CONFIG = {
  ...
  readOnlyBashPatterns: [
    ...
    // mortise CLI read-only
    { regex: /^mortise\s+label\s+(list|get)\b/, source: '^mortise\\s+label\\s+(list|get)\\b', comment: 'mortise label read-only operations' },
    { regex: /^mortise\s+source\s+(list|get|validate|test)\b/, source: '^mortise\\s+source\\s+(list|get|validate|test)\\b', comment: 'mortise source read-only operations' },
    { regex: /^mortise\s+skill\s+(list|get|validate|where)\b/, source: '^mortise\\s+skill\\s+(list|get|validate|where)\\b', comment: 'mortise skill read-only operations' },
    { regex: /^mortise\s+automation\s+(list|get|validate|history|last-executed|test|lint)\b/, source: '^mortise\\s+automation\\s+(list|get|validate|history|last-executed|test|lint)\\b', comment: 'mortise automation read-only operations' },
    { regex: /^mortise\s*$/, source: '^mortise\\s*$', comment: 'mortise bare invocation' },
    { regex: /^mortise\s+(label|source|skill|automation)\s+--help\b/, source: '^mortise\\s+(label|source|skill|automation)\\s+--help\\b', comment: 'mortise entity help flags' },
    { regex: /^mortise\s+--(help|version|discover)\b/, source: '^mortise\\s+--(help|version|discover)\\b', comment: 'mortise global flags' },
...
// ============================================================
// Group 27: mortise CLI allowlist
// ============================================================

describe('ShellGuard corpus: mortise CLI allowlist', () => {
  const shouldAllow = [
    'mortise',
    'mortise --help',
    'mortise --version',
    ...
    'mortise skill --help',
    'mortise skill list',
    'mortise skill get commit-helper',
    'mortise skill where commit-helper',
    'mortise skill validate commit-helper',
    'mortise automation list',
    'mortise automation get abc123',
    'mortise automation validate',
    'mortise automation history abc123 --limit 5',
    'mortise automation last-executed abc123',
    'mortise automation test abc123 --match "x"',
    'mortise automation lint',
  ];

  const shouldBlock = [
    'mortise label create --name Bug',
    'mortise label update bug --name "Bug Report"',
    'mortise label delete bug',
    'mortise label move bug --parent root',
    'mortise label reorder --parent root a b c',
    'mortise source create --name Linear --provider linear --type mcp',
    'mortise source update linear --json "{\"enabled\":false}"',
    'mortise source delete linear',
    'mortise skill create --name "Review" --description "x"',
    'mortise skill update review --json "{\"description\":\"y\"}"',
    'mortise skill delete review',
    'mortise automation create --event UserPromptSubmit --prompt "x"',
    'mortise automation update abc123 --json "{\"enabled\":false}"',
    'mortise automation delete abc123',
    'mortise automation enable abc123',
    'mortise automation disable abc123',
