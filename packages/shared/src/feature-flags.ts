 ...
 *
 * 对应配置项 `piExtensions.delegatePromptAutomation`，默认关闭（由 mortise 的
 * PromptHandler 自行创建会话）。启用后 prompt action 走 pi prompt-automation
 * 扩展（通过 extension_command_invoke RPC，由 Task 2 桥接层实现）。
 * 覆盖方式：MORTISE_FEATURE_DELEGATE_PROMPT_AUTOMATION=1|0
 */
export function isDelegatePromptAutomationEnabled(): boolean {
  const override = parseBooleanEnv(getEnv('MORTISE_FEATURE_DELEGATE_PROMPT_AUTOMATION'));
  if (override !== undefined) return override;
  return false;
}
