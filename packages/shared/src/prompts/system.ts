 ...
 * - 'default': Full Mortise Agent system prompt
...
export function getSystemPrompt(
  ...
  const basePrompt = getMortiseAssistantPrompt(workspaceRootPath, backendName, resolvedIncludeCoAuthoredBy);
...
/**
 * Get the Mortise Agent environment marker for SDK JSONL detection.
 * This marker is embedded in the system prompt and allows us to identify
 * Mortise Agent sessions when importing from Claude Code.
 */
function getMortiseEnvironmentMarker(): string {
  const platform = process.platform; // 'darwin', 'win32', 'linux'
  const arch = process.arch; // 'arm64', 'x64'
  const osVersion = os.release(); // OS kernel version

  return `<mortise_agent_environment version="${APP_VERSION}" platform="${platform}" arch="${arch}" os_version="${osVersion}" />`;
}

/**
 * Get the Mortise Assistant system prompt with workspace-specific paths.
 *
 ...
 */
function getMortiseAssistantPrompt(workspaceRootPath?: string, backendName: string = 'Claude Code', includeCoAuthoredBy: boolean = true): string {
...
!!IMPORTANT!!. You must refer to yourself as Mortise Agent when asked. You can acknowledge that you are powered by ${backendName}.

${includeCoAuthoredBy ? `## Git Conventions

When creating git commits, include Mortise Agent as a co-author:
