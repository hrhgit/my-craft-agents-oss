#!/usr/bin/env bun
/**
 * Debug script to print the full Mortise Agent system prompt with annotations.
...
${colors.bgMagenta}${colors.bold}                    MORTISE AGENT SYSTEM PROMPT BREAKDOWN                         ${colors.reset}
${colors.bgMagenta}${colors.bold}                                                                                ${colors.reset}
`);
...
const systemPrompt = getSystemPrompt(
  ...
  '/Users/example/.mortise/workspaces/abc123' // Example workspace path
);
...
const systemPromptWithDebug = getSystemPrompt(
  undefined,
  { enabled: true, logFilePath: '~/Library/Logs/@mortise/electron/main.log' },
  '/Users/example/.mortise/workspaces/abc123'
);
...
printAnnotation('');
printAnnotation('Volatile vs stable (issue #862): blocks 1-2 (date/time, session_state)');
printAnnotation('change per turn and are VOLATILE; blocks 3-4 (workspace capabilities, working');
...
printSection(
  '4. WORKING DIRECTORY - getWorkingDirectoryContext()',
...
printSection(
  '5. RECOVERY CONTEXT - buildRecoveryContext() [only on resume]',
...
${colors.bold}SDK Configuration:${colors.reset}
  ...
  systemPrompt.append: getSystemPrompt() ${colors.dim}// Mortise Agent additions (static, cacheable)${colors.reset}
...
${colors.bold}Dynamic User Message Components (per message):${colors.reset}
  ...
  ${colors.dim}Claude: 1-4 ride the user tail. Pi (#862): STABLE 3-4 -> system prefix, VOLATILE 1-2 -> user tail.${colors.reset}
