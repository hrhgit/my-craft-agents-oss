...
export async function installArchive(params: { archiveData: ArrayBuffer, version: string }): Promise<void> {
  ...
  const versionDirectory = join(homedir(), '.local', 'share', 'mortise', 'versions', version);
  const binaryPath = join(versionDirectory, 'mortise');
  const symlinkDirectory = join(homedir(), '.local', 'bin');
  const symlinkPath = join(symlinkDirectory, 'mortise');
