...
describe('Windows packaging size contract', () => {
  it('uses an allowlist and stages the Pi runtime only once', () => {
    ...
    expect(config.win.files[0]).toBe('dist/**/*');
    expect(config.win.files).toContain('!dist/resources/pi-runtime/**/*');
    expect(config.files).toContain('!dist/resources/pi-runtime/**/*');
    expect(config.win.files).toContain('!dist/resources/bin/**/*');
    expect(config.win.files).toContain('!dist/resources/session-mcp-server/**/*');
