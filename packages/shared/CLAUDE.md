...
- OAuth identity (account/org) is captured from the token-exchange response by the caller and persisted on `LlmConnection` (`oauthAccountUuid/Email`, `oauthOrganizationUuid/Name`, `oauthProfileVerifiedAt`) by threading it through the `SETUP_LLM_CONNECTION` payload (`oauthIdentity`), **not** the EXCHANGE handler — the connection record is created by SETUP, which runs after the exchange. `updateLlmConnection` rebuilds connections from a hardcoded allowlist, so any new persisted field must be added there too or it is dropped on the next save (#838).
...
Two checks gate i18n correctness and are wired into `validate:ci`:
...
`parity` alone is insufficient — it can't detect symmetric losses across all locales (a merge that drops the same 50 keys from every locale file passes parity but breaks the UI). Literal callsite coverage for `t(...)` / `i18n.t(...)` / `<Trans i18nKey>` should be restored as a real script before it is re-added to `validate:ci`.

When resolving locale merge conflicts, run `bun run validate:ci` and trust the wired checks for sortedness and parity. Until literal callsite coverage is restored, manually audit any change that removes or renames translation keys across every locale.
