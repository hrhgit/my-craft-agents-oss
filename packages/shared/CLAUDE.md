...
- Craft's global `midStreamBehavior` controls whether mid-stream user sends steer the in-flight turn or wait for the next turn. It is independent of provider selection and is read via `getMidStreamBehavior()` in `SessionManager.sendMessage`.
