...
export interface ResolveScriptRuntimeContext {
  /**
   * Whether host app is packaged. Defaults to MORTISE_IS_PACKAGED=1.
...
function resolveResourcesBase(ctx?: ResolveScriptRuntimeContext): string | null {
  const explicit = ctx?.resourcesBasePath || process.env.MORTISE_RESOURCES_BASE;
...
function resolveAppRoot(ctx?: ResolveScriptRuntimeContext): string | null {
  const explicit = ctx?.appRootPath || process.env.MORTISE_APP_ROOT;
...
function validatePackagedEnvRuntime(command: string, label: string): string {
  ...
  if (!isAbsolute(command) && !hasPathSeparator) {
    throw new Error(
      ...
      'Packaged builds do not allow PATH-based runtime resolution. Configure an absolute MORTISE_* path or ship a bundled runtime.'
    );
  }
  ...
  if (!existsSync(resolvedCommand)) {
    throw new Error(
      ...
      'Configure a valid absolute MORTISE_* path or ship a bundled runtime.'
    );
  }
 ...
 *
 * Resolution order:
 * - env override (MORTISE_UV / MORTISE_NODE / MORTISE_BUN)
...
export function resolveScriptRuntime(
  ...
  if (language === 'python3') {
    if (process.env.MORTISE_UV) {
      const cmd = isPackaged
        ? validatePackagedEnvRuntime(process.env.MORTISE_UV, 'Python/uv')
        : process.env.MORTISE_UV;
    ...
    throw new Error(
      isPackaged
        ...
        : 'Python runtime unavailable: uv was not found. Configure MORTISE_UV or install uv on PATH.'
    );
  }

  if (language === 'node') {
    if (process.env.MORTISE_NODE) {
      const cmd = isPackaged
        ? validatePackagedEnvRuntime(process.env.MORTISE_NODE, 'Node')
        : process.env.MORTISE_NODE;
    ...
    throw new Error(
      isPackaged
        ...
        : 'Node runtime unavailable: configure MORTISE_NODE or install node on PATH.'
    );
  }

  if (process.env.MORTISE_BUN) {
    const cmd = isPackaged
      ? validatePackagedEnvRuntime(process.env.MORTISE_BUN, 'Bun')
  ...
  throw new Error(
    isPackaged
      ...
      : 'Bun runtime unavailable: configure MORTISE_BUN or install bun on PATH.'
  );
}
