...
export function ActivityCardsOverlay({
  ...
  const jsonTheme = useMemo(() => (theme === 'dark' ? mortiseAgentDarkTheme : mortiseAgentLightTheme), [theme])
          ...
          <div className="flex-1 overflow-y-auto min-h-0 p-4 space-y-4">
            {isInputCard && commandPreview && (
              <div className="bg-background shadow-minimal rounded-[8px] px-4 py-3 font-mono">
                <div className="text-xs font-semibold text-muted-foreground/70 mb-1">{t('overlay.activityCards.command')}</div>
            ...
            <div>
              {isInputCard && (
                <div className="text-xs font-semibold text-muted-foreground/70 mb-2 px-1">{t('overlay.activityCards.inputParams')}</div>
              )}
              <div className="p-4">
  ...
  return (
    <PreviewOverlay
      ...
      onClose={onClose}
      theme={theme}
      typeBadge={{ icon: Layers, label: t('overlay.activity'), variant: 'blue' }}
      title={title}
