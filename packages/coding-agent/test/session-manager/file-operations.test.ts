...
describe("findMostRecentSession", () => {
	...
	it("accepts session headers longer than 512 bytes", () => {
		const longParentSessionPath = join(
			tempDir,
			"nested".repeat(40),
			"another".repeat(40),
			"session-with-a-very-long-parent-path.jsonl",
		);
		...
		writeFileSync(
			...
			`${JSON.stringify({
				...
				cwd: tempDir,
				parentSession: longParentSessionPath,
			})}\n`,
		);

		expect(readFileSync(file, "utf8").split("\n")[0]?.length).toBeGreaterThan(512);
		expect(findMostRecentSession(tempDir)).toBe(file);
	});
});
