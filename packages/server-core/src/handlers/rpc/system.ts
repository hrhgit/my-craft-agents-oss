...
function parseInternalMortiseDeepLink(parsed: URL): ParsedInternalDeepLink | null {
  if (parsed.protocol !== 'mortise:') return null
  ...
  server.handle(RPC_CHANNELS.shell.OPEN_URL, async (ctx, url: string) => {
    ...
    try {
      ...
      if (classification.kind === 'internal-deeplink') {
        const deepLink = parseInternalMortiseDeepLink(parsed)
        ...
        if (deepLink?.navigation?.view || deepLink?.navigation?.action) {
          ...
          deps.platform.logger.info('[OPEN_URL] Routing mortise:// URL internally via deeplink:navigate')
