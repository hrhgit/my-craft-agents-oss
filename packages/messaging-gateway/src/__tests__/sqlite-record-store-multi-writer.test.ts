...
const worker = join(import.meta.dir, 'fixtures', 'sqlite-record-store-worker.ts')
...
describe('SqliteRecordStore multi-writer persistence', () => {
  it('merges disjoint object updates from separate backend processes', async () => {
    const directory = mkdtempSync(join(tmpdir(), 'mortise-sqlite-record-store-'))
