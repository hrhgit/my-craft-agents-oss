#!/usr/bin/env bash
# =============================================================================
# Docker smoke test for Mortise Server
#
...
while [ "$ELAPSED" -lt "$TIMEOUT" ]; do
  ...
  if docker logs "$CONTAINER_NAME" 2>&1 | grep -q "MORTISE_SERVER_URL="; then
    READY=true
    break
  fi
