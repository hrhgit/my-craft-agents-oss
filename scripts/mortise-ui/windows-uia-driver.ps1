...
if ($request.action -eq 'focus') {
  ...
  if ($target.TryGetCurrentPattern([System.Windows.Automation.InvokePattern]::Pattern, [ref]$pattern)) {
    ...
    Add-Type @'
using System.Runtime.InteropServices;
public static class MortiseUiMouse {
...
'@
    [MortiseUiMouse]::SetCursorPos([int]$point.X, [int]$point.Y) | Out-Null
    [MortiseUiMouse]::mouse_event(0x0002, 0, 0, 0, [UIntPtr]::Zero)
    [MortiseUiMouse]::mouse_event(0x0004, 0, 0, 0, [UIntPtr]::Zero)
  }
