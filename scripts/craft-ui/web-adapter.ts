...
const runId = requiredEnv('CRAFT_UI_RUN_ID')
const token = requiredEnv('CRAFT_UI_TOKEN')
const endpointManifestPath = resolve(requiredEnv('CRAFT_UI_ENDPOINT_MANIFEST'))
const artifactsDir = resolve(requiredEnv('CRAFT_UI_ARTIFACTS_DIR'))
...
if (process.env.CRAFT_UI_TEST_HOST !== '1' || process.env.NODE_ENV === 'production') {
  throw new Error('The WebUI validation adapter is source-development only.')
}
if (process.env.CRAFT_UI_SURFACE !== 'webui') throw new Error('WebUI adapter requires CRAFT_UI_SURFACE=webui.')
if (process.env.CRAFT_UI_PROTOCOL_VERSION !== String(UI_VALIDATION_PROTOCOL_VERSION)) throw new Error('Unsupported UI validation protocol version.')
...
try {
  const commonEnv = {
    ...process.env,
    CRAFT_SERVER_TOKEN: serverSecret,
    CRAFT_RPC_HOST: '127.0.0.1',
    CRAFT_RPC_PORT: String(rpcPort),
    CRAFT_WEBUI_PORT: String(webPort),
    CRAFT_WEBUI_HOST: '127.0.0.1',
    CRAFT_WEBUI_AUTO_LOGIN: 'true',
    CRAFT_WEBUI_DIR: join(repoRoot, 'apps', 'webui', 'src'),
    CRAFT_WEBUI_WS_URL: `ws://127.0.0.1:${rpcPort}`,
    CRAFT_BUNDLED_ASSETS_ROOT: join(repoRoot, 'apps', 'electron'),
    CRAFT_DEBUG: 'true',
  }
  ...
  await context.addInitScript(() => {
    ...
    const state = {
      ...
      publishState(batch: { version: number; states: Array<Record<string, unknown>> }) {
        ...
        state.revision += 1
        dispatchEvent(new CustomEvent('craft:ui-validation:state-change', { detail: { revision: scoped.revision, seq: scoped.seq } }))
      },
      dispose() {
    ...
    Object.defineProperty(window, '__craftUiValidation', { value: state, configurable: false, enumerable: false })
...
async function rendererStateSnapshot(): Promise<WebRendererStateSnapshot> {
  return await activePage().evaluate(() => {
    const host = window.__craftUiValidation as typeof window.__craftUiValidation & { stateSnapshot?: () => WebRendererStateSnapshot }
...
async function openRoute(params: Record<string, unknown>): Promise<unknown> {
  ...
  if (route.surface === 'chat' && (route.sessionId || route.workspaceId)) {
    ...
    const nav = browserPage.locator(`[data-craft-semantic-id=${JSON.stringify(semanticIds[route.surface])}]`)
    ...
    if (route.section) {
      ...
      const section = browserPage.locator(`[data-craft-semantic-id=${JSON.stringify(sectionId)}]`)
...
async function appShellScenarioCommand(method: WebScenarioMethod, input?: unknown): Promise<unknown> {
  ...
  return await activePage().evaluate(async ({ method, input }) => {
    const bridge = (globalThis as Record<string, any>).__CRAFT_UI_VALIDATION_APP_SHELL_SCENARIOS_V1__
...
async function snapshot(): Promise<SemanticSnapshot> {
  const raw = await activePage().evaluate(() => {
    const state = (window as unknown as { __craftUiValidation?: { revision: number; snapshot?: (options?: { maxNodes?: number; maxStringLength?: number }) => unknown } }).__craftUiValidation
...
async function extensionDefinitionSnapshot(target: Record<string, unknown>): Promise<unknown> {
  ...
  const definitions = await activePage().evaluate(async filter => {
    const bridge = (globalThis as unknown as Record<string, unknown>).__CRAFT_UI_VALIDATION_EXTENSION_BRIDGE_V1__ as {
...
async function action(raw: Record<string, unknown>, signal?: AbortSignal): Promise<unknown> {
  ...
  if (mode === 'semantic') {
    ...
    await activePage().evaluate(async semanticRequest => {
      const bridge = (window as unknown as { __craftUiValidation?: { action?: (request: typeof semanticRequest) => Promise<unknown> } }).__craftUiValidation
  ...
  const stateEvents = await activePage().evaluate(afterSeq => {
    const host = window.__craftUiValidation as typeof window.__craftUiValidation & { readStateEvents?: (cursor: number) => { latestSeq: number; events: Array<Record<string, unknown>> } }
...
async function evaluatePredicate(predicate: UiValidationWaitPredicate, afterSeq = 0): Promise<{ matched: boolean; value: unknown }> {
  ...
  if (predicate.kind === 'event') {
    const result = await activePage().evaluate(({ type, after }) => {
      const host = window.__craftUiValidation as typeof window.__craftUiValidation & { readStateEvents?: (afterSeq: number) => { latestSeq: number; events: Array<Record<string, unknown>>; droppedBeforeSeq?: number } }
  ...
  for (const kind of request.include) {
    ...
    if (kind === 'screenshot') await add(kind, 'screenshot.png', 'image/png', path => activePage().screenshot({ path, fullPage: true }).then(() => undefined))
      ...
      const events = await activePage().evaluate(cursor => {
        const host = window.__craftUiValidation as typeof window.__craftUiValidation & { readStateEvents?: (after: number) => unknown }
...
async function liveRevision(): Promise<number> {
  ...
  return page.evaluate(() => (window as unknown as { __craftUiValidation?: { revision: number } }).__craftUiValidation?.revision ?? 0)
}
...
async function waitForPageSignal(baseline: WebWaitBaseline, timeoutMs: number, signal?: AbortSignal): Promise<void> {
  ...
  const cancelEvent = `craft:ui-validation:wait-cancel:${randomUUID()}`
  const pageWait = activePage().evaluate(({ baseline, timeoutMs, cancelEvent }) => new Promise<'changed' | 'timeout' | 'cancelled'>(resolveWait => {
    ...
    const finish = (result: 'changed' | 'timeout' | 'cancelled') => {
      ...
      observer.disconnect()
      window.removeEventListener('craft:ui-validation:semantic-change', changed)
      window.removeEventListener('craft:ui-validation:state-change', changed)
    ...
    const current = () => {
      const host = window.__craftUiValidation as typeof window.__craftUiValidation & { stateSnapshot?: () => { revision?: number; latestSeq?: number } }
    ...
    window.addEventListener('craft:ui-validation:semantic-change', changed, { once: true })
    window.addEventListener('craft:ui-validation:state-change', changed, { once: true })
...
function findBrowserExecutable(): string {
  const configured = process.env.CRAFT_UI_BROWSER_EXECUTABLE
  ...
  if (!found) throw new Error('No Chromium browser found. Set CRAFT_UI_BROWSER_EXECUTABLE.')
  return found
}
...
async function runtimeLog(): Promise<string> {
  const path = join(craftConfigDir, 'logs', 'runtime.log')
