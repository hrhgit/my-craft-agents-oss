...
export const CRAFT_UI_FIXTURE_VERSION = 1 as const
...
export interface CraftUiFixtureFile {
  path: string
  content: string
}

export interface CraftUiFixtureMessage {
  id?: string
...
export interface CraftUiFixtureSession {
  id: string
  ...
  hasUnread?: boolean
  hidden?: boolean
  messages?: CraftUiFixtureMessage[]
  /** Paths are relative to the session sidecar and must start with a supported sidecar folder. */
  files?: CraftUiFixtureFile[]
}

export interface CraftUiFixtureWorkspace {
  id: string
  name: string
  slug?: string
  permissionMode?: 'safe' | 'ask' | 'allow-all'
  files?: CraftUiFixtureFile[]
  sessions?: CraftUiFixtureSession[]
}

export interface CraftUiFixtureSpec {
  version: typeof CRAFT_UI_FIXTURE_VERSION
  active?: { workspaceId: string; sessionId?: string | null }
  workspaces: CraftUiFixtureWorkspace[]
}

export interface CraftUiFixtureSummary {
  version: typeof CRAFT_UI_FIXTURE_VERSION
...
export const CRAFT_UI_FIXTURE_SCHEMA = {
  $schema: 'https://json-schema.org/draft/2020-12/schema',
  $id: 'https://craft-agent.local/schemas/craft-ui-fixture-v1.json',
  title: 'Craft UI fixture',
  ...
  properties: {
    version: { const: CRAFT_UI_FIXTURE_VERSION },
    active: {
...
export const DEFAULT_CRAFT_UI_FIXTURE: CraftUiFixtureSpec = {
  version: CRAFT_UI_FIXTURE_VERSION,
      ...
      sessions: [
        ...
        },
        {
          ...
          messages: [
            ...
            { role: 'assistant', content: 'Suggested copy: Choose a workspace to begin. Craft keeps project files local unless you explicitly connect a remote service.', timestamp: fixtureMinute(42) },
          ],
        },
      ],
    },
...
export function loadCraftUiFixtureSpec(path: string): CraftUiFixtureSpec {
  ...
  return validateCraftUiFixtureSpec(parsed)
}

export function validateCraftUiFixtureSpec(value: unknown): CraftUiFixtureSpec {
  const root = record(value, 'fixture')
  exactKeys(root, ['version', 'active', 'workspaces'], 'fixture')
  if (root.version !== CRAFT_UI_FIXTURE_VERSION) fail('fixture.version', `must equal ${CRAFT_UI_FIXTURE_VERSION}`)
  ...
  const workspaces = rawWorkspaces.map((raw, workspaceIndex): CraftUiFixtureWorkspace => {
    ...
    const sessions = rawSessions.map((rawSession, sessionIndex): CraftUiFixtureSession => {
  ...
  return { version: CRAFT_UI_FIXTURE_VERSION, active, workspaces }
}

export function summarizeCraftUiFixture(spec: CraftUiFixtureSpec): CraftUiFixtureSummary {
  return {
...
function validateActive(value: unknown, workspaces: Set<string>, sessions: Map<string, string>, fallbackWorkspaceId: string): CraftUiFixtureSpec['active'] {
...
function validateFiles(value: unknown, path: string, scope: 'workspace' | 'session'): CraftUiFixtureFile[] {
  ...
  return array(value, path, 0, LIMITS.files).map((raw, index) => {
    ...
    if (scope === 'workspace' && (comparisonPath === 'config.json' || comparisonPath.startsWith('.craft/'))) {
...
function validateMessages(value: unknown, path: string): CraftUiFixtureMessage[] {
  if (value === undefined) return []
  ...
  return array(value, path, 0, LIMITS.messagesPerSession).map((raw, index) => {
    ...
    return compact({
      id,
      role: item.role as CraftUiFixtureMessage['role'],
...
function optionalPermissionMode(value: unknown, path: string): CraftUiFixtureWorkspace['permissionMode'] {
  if (value === undefined) return undefined
  if (typeof value !== 'string' || !PERMISSION_MODES.has(value)) fail(path, 'must be safe, ask, or allow-all')
  return value as CraftUiFixtureWorkspace['permissionMode']
}
...
function bytesOfFiles(files: CraftUiFixtureFile[]): number {
