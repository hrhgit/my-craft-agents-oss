...
import {
  DEFAULT_CRAFT_UI_FIXTURE,
  summarizeCraftUiFixture,
  validateCraftUiFixtureSpec,
  type CraftUiFixtureFile,
  type CraftUiFixtureSpec,
  type CraftUiFixtureSummary,
} from './fixture.ts'
...
function writeFixtureFile(basePath: string, file: CraftUiFixtureFile): void {
  const filePath = join(basePath, ...file.path.split('/'))
  mkdirSync(dirname(filePath), { recursive: true })
  writeFileSync(filePath, file.content, 'utf8')
}

function seedFixtureProfile(root: string, craftConfigDir: string, piAgentDir: string, input?: CraftUiFixtureSpec): CraftUiFixtureSummary {
  const spec = validateCraftUiFixtureSpec(input ?? DEFAULT_CRAFT_UI_FIXTURE)
  ...
  writeJson(join(craftConfigDir, 'config.json'), {
  ...
  writeJson(join(craftConfigDir, 'preferences.json'), {
    name: 'Craft UI Tester',
    timezone: 'UTC',
    notes: 'Disposable profile generated by craft-ui.',
    updatedAt: FIXTURE_CREATED_AT,
  })
  return summarizeCraftUiFixture(spec)
}
...
export function prepareProfile(args: {
  ...
  const fixture = args.mode === 'fixture' ? seedFixtureProfile(root, craftConfigDir, piAgentDir, args.fixtureSpec) : undefined
  return { root, craftConfigDir, piAgentDir, electronUserDataDir, mode: args.mode, containsClonedUserData: args.mode === 'clone', fixture }
}
