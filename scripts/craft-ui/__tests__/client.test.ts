...
import { CraftUiClientError, createCraftUiSurfaceDriver, readEndpointManifest, requestCraftUiHost } from '../client.ts'
import { CRAFT_UI_PROTOCOL_VERSION } from '../protocol.ts'
...
describe('craft-ui endpoint validation', () => {
  it('rejects non-loopback and hostname-based endpoints', () => {
    const root = mkdtempSync(join(tmpdir(), 'craft-ui-client-')); roots.push(root)
    for (const url of ['http://192.168.1.4:9999', 'https://127.0.0.1:9999', 'http://localhost:9999']) {
      ...
      expect(() => readEndpointManifest(path)).toThrow(CraftUiClientError)
    }
  })
  ...
  it('rejects a response whose requestId does not match the request', async () => {
    ...
    const server = Bun.serve({
      ...
      async fetch(request) {
        ...
        return Response.json({
          v: CRAFT_UI_PROTOCOL_VERSION, kind: 'response', id: body.id, requestId: 'different-request', runId,
    ...
    writeFileSync(endpointManifestPath, JSON.stringify({
      protocolVersion: CRAFT_UI_PROTOCOL_VERSION, runId, surface: 'webui', transport: 'http',
      url: `http://127.0.0.1:${server.port}`, pid: process.pid, readyAt: new Date().toISOString(),
    }))
    await expect(requestCraftUiHost({ endpointManifestPath, tokenPath, runId, command: 'app.status' }))
      .rejects.toMatchObject({ code: 'INVALID_RESPONSE' })
  })
})

describe('CraftUiSurfaceDriver facade', () => {
  it('maps the complete driver contract to bounded JSON protocol commands', async () => {
    ...
    let seq = 0
    const driver = createCraftUiSurfaceDriver({
