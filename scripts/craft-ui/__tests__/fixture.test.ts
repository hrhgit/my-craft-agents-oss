...
import { CRAFT_UI_FIXTURE_SCHEMA, validateCraftUiFixtureSpec } from '../fixture.ts'

describe('craft-ui fixture specification', () => {
  it('publishes the bounded workspace/session/history schema', () => {
    expect(CRAFT_UI_FIXTURE_SCHEMA).toMatchObject({
      properties: {
        version: { const: 1 },
        workspaces: { maxItems: 32 },
      },
    })
    expect(CRAFT_UI_FIXTURE_SCHEMA.examples[0].workspaces[0].sessions[0].messages).toHaveLength(2)
  })

  it('normalizes a composable data scene', () => {
    expect(validateCraftUiFixtureSpec({
      version: 1,
