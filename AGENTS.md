...
- When hands-on validation would be cumbersome or unstable for AI but straightforward for a human, provide the exact verification steps and ask the human to run them instead of automating the check.
- Performance and reliability fixes should repair the shared data or state boundary, define the invariant there, and add regression coverage; avoid patches that only mask the visible UI symptom.
- Treat the future TODO document as a passive idea record: do not use it as implementation scope or decision guidance unless the user explicitly asks to invoke or reference it.
- When implementing or revising slash commands, hide or disable commands whose capabilities already have Craft GUI controls; keep slash commands for capabilities without a GUI entry.
...
- Craft extension authoring documentation follows Pi's extension-guide style: lead with a runnable quick start, organize behavior by user-facing API, keep constraints beside the relevant API, include complete examples and an examples index, and leave architecture rationale in separate architecture documents.
- Craft UI primitives must expose stable semantic identity, actions, and state for automated validation. Prefer accessible roles and names, add stable test IDs only when those semantics are insufficient, and provide external agents with a bounded structured UI snapshot rather than raw DOM internals.
