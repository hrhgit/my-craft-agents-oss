...
# 获取所有会话文件
$sessionFiles = Get-ChildItem -Path $SessionPath -Filter "*.jsonl" | Sort-Object LastWriteTime -Descending

Write-Host "找到 $($sessionFiles.Count) 个会话文件"
...
foreach ($file in $sessionFiles) {
    ...
    for ($i = 0; $i -lt $messages.Count; $i++) {
        ...
        if ($msg.message.role -eq "toolResult") {
            ...
            $endIdx = [Math]::Min($messages.Count - 1, $i + 3)
