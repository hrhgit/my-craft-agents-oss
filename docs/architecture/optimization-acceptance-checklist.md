...
8. Runtime or physical evidence is proportional to the affected surface. Electron/native changes require Electron/native evidence; WebUI evidence is used only for supported WebUI behavior.
...
|---|---|---|---|---|
| OPT-010 | P1 | needs-fix | `session-lifecycle`: workspace root is the only working-directory authority. Retain typed rejection of removed `session.workingDirectory`; remove its silent scrubber, obsolete mutator, and extra settings field; remove or rewrite legacy-contract mocks/tests while preserving or increasing typed-rejection and workspace-resolution coverage. | Complete `SES-E1-SL`, then `SES-E1-AS` and `SES-E1-ND`; pass the integrated caller scan and Session contract tests, then prove file/tool/skill resolution follows the selected workspace. |
...
| OPT-014 | P1 | needs-fix | Workspace manifests are the sole dependency declarations and the canonical graph validator enforces manifest, build, and source-import DAGs. `session-tools-core` is intentionally host-neutral and may be consumed by `shared`; the forbidden edge is `session-tools-core -> shared`. Do not extract another package. | Prepare `EXT-BR-01-HSC`, complete `EXT-BR-01-BR` in the private combined worktree, and publish both manifests plus lockfile atomically; complete `EXT-BR-02`; retain the reverse-edge guard; pass frozen clean-checkout installation, graph, production bundle/metafile, and packaged/source-dev initialization evidence. |
...
| OPT-016 | P1 | needs-fix | Each row in [`legacy-cleanup-inventory.md`](./legacy-cleanup-inventory.md) is routed to its exact owner; Mortise runtime reads and writes only Mortise roots and historical Sessions are not imported. | Atomically integrate `EXT-LEG-01-ER` + `EXT-LEG-01-CFG`, complete `EXT-LEG-01-PI` and `EXT-LEG-02`; resolve every remaining inventory row by exact path/symbol allowlist; scan tracked, untracked, ignored, generated, and packaged production surfaces; then pass clean-start proof. Data deletion remains separately confirmation-gated by exact manifests. |
