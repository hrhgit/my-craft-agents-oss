...
|---|---|---|---|---|
| OPT-010 | P1 | needs-fix | `session-lifecycle`: workspace root is the only working-directory authority. Retain typed rejection of removed `session.workingDirectory`; remove its silent scrubber, obsolete mutator, and extra settings field; remove or rewrite legacy-contract mocks/tests while preserving or increasing typed-rejection and workspace-resolution coverage. | The three owner patches are present and focused gates passed; the primary completes `SES-P2-INT` to prove selected-workspace file/skill authority, unpublished draft behavior, first-assistant publication, persistence state, and cleanup through isolated source Electron without bypassing the typed fixture boundary. |
...
| OPT-016 | P1 | needs-fix | Each row in [`legacy-cleanup-inventory.md`](./legacy-cleanup-inventory.md) is routed to its exact owner; Mortise runtime reads and writes only Mortise roots and historical Sessions are not imported. | Shared/config/Pi aliases and the Craft-era local identifier are removed, but `SpawnSessionSchema` still accepts `max` and omits `minimal`; complete `EXT-LEG-01-ST`, then run the exact-path zero-alias inventory scan and clean-start proof. Data deletion remains separately confirmation-gated by exact manifests. |
