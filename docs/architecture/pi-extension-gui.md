...
- Recommendation: `agent-product-first + reuse-first`。placement 和信息层级优先参考当前主流 Agent 桌面端；隔离、生命周期和 dock 机制再复用成熟插件系统经验，Mortise 自行实现轻量宿主。
- Primary UX references: [OpenAI Codex](https://openai.com/codex/)、[OpenAI Apps SDK UI guidelines](https://developers.openai.com/apps-sdk/concepts/ui-guidelines/)、[Claude Artifacts](https://claude.com/blog/artifacts-in-claude-code) 以及 Claude Desktop / Claude Code Desktop 的一手产品资料。
- Secondary implementation reference: VS Code Workbench Extension API，只用于受控 webview、extension host 和 movable dock 的边界，不决定 conversation GUI 的默认位置。

Mortise GUI 必须跟随 Pi 扩展分发，而不是由 Mortise 按扩展名称硬编码。扩展通过 `ctx.ui.upsertContribution()` 发布可序列化 UI，Mortise 校验后将它放入宿主拥有的 surface。conversation、tool、composer 和 inline 区域提供最高自由度；侧栏、导航和窗口热点使用容量受控槽位；完整工具进入 universal dock 的 `workspace.content`。任意 DOM 使用同一协议中的沙箱 UI App，而不是让扩展脚本进入 Mortise renderer。
...
| Eclipse Theia Platform | 96.75 | pass | L2 | 平台规模远大于 Mortise 所需，直接引入成本过高 | [GitHub](https://github.com/eclipse-theia/theia), [Plugin Authoring](https://theia-ide.org/docs/authoring_plugins/) |
...
- Theia 明确区分 browser、Electron 和 backend，符合 Mortise WebUI/Electron 共用 renderer、平台能力走 adapter 的现状。
- 三个候选均通过 18 个月维护、许可证和官方来源硬门槛；确定性评分脚本结果依次为 99.75、97.25、96.75。

第二阶段针对沙箱宿主重新按相同规则评分：VS Code Webview / Extension Host 99.90，Backstage Frontend Extensions 97.35，Eclipse Theia Plugin Host 96.90。结论仍是复用 VS Code 的隔离边界和生命周期模式，但 Mortise 使用浏览器原生 `iframe sandbox + CSP + MessageChannel` 自行实现轻量宿主，以保持 WebUI/Electron 同构。
...
以下基础能力已经完成：Pi `ExtensionUIContext` 的 contribution API 和 revision 生命周期、包括 `workspace.content`/`workspaceContent` 在内的公共类型导出、Mortise registry/layout manager/host primitives、主对话/输入框/侧栏 surface，以及 universal dock 的 workspace content host。

后续修改必须保持以下约束：

1. 扩展 GUI 继续通过版本化 contribution 协议发布；不得重新引入按扩展 ID、widget key 或业务功能分支的 Mortise renderer。
...
- `composer.above`, `composer.below`, `composer.replace`

Composer compact slots:

- `composer.toolbar`, `composer.status`

High-freedom workspace content surface:

- `workspace.content` makes an ordinary content tool available to the workspace's universal dock. The user opens it from the content picker or restores a saved tab; publication alone does not steal focus. Mortise owns tab placement, selection, sizing, focus, permissions, saved layout and recovery; the extension owns the isolated tab body. Version 1 `workspaceContent` metadata declares a host-rendered title/icon, session/workspace/global scope, singleton/multiple instance policy and an initial `active`/`adjacent` group preference. It never declares a persistent direction or width.
...
Replace surfaces have at most one deterministic winner. If the winning contribution is absent or invalid, Mortise keeps the built-in UI; sandbox replacements keep the built-in UI until the sandbox reports ready.
...
Each contribution may declare `priority`, `order`, `group`, `collapse`, `overflow`, and `exclusive`. The current `SurfaceLayoutManager` applies these rules deterministically:
...
|---|---:|---|
| Timeline / inline panels | unbounded host-rendered items; 4 mounted sandbox apps | vertical flow plus sandbox overflow |
...
| Sidebar header/section/footer | 2 / 5 / 2 | host overflow |
...
- Definitions carry stable IDs, readiness signals, command-backed actions, bounded scenarios, and semantic snapshots. Pi owns monotonically increasing validation revisions; Mortise replaces untrusted route identity with its trusted session/runtime/extension route before rendering.
