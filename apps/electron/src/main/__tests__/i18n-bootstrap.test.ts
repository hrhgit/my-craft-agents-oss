 ...
 * subprocess with `MORTISE_CONFIG_DIR` set in its env (same pattern as
  ...
  it('hydrates main i18n from persisted uiLanguage', () => {
    ...
    try {
      const r = runScript(
        configDir,
        `
          import { setupI18n, i18n } from '@mortise/shared/i18n';
          import { setPersistedUiLanguage, getPersistedUiLanguage } from '@mortise/shared/config';
          setupI18n();
  ...
  it('returns undefined when no language is persisted (no hydration step)', () => {
    ...
    try {
      const r = runScript(
        ...
        `
          import { setupI18n, i18n } from '@mortise/shared/i18n';
          import { getPersistedUiLanguage } from '@mortise/shared/config';
          setupI18n();
          const persisted = getPersistedUiLanguage();
