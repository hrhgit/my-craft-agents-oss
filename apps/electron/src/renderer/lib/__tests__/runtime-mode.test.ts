...
describe('Craft runtime mode', () => {
  ...
  it('allows an explicit URL override in either direction', () => {
    expect(resolveCraftTestMode({ search: '?craftTestMode=1', viteTestMode: '0' })).toBe(true)
    expect(resolveCraftTestMode({ search: '?craftTestMode=0', viteTestMode: '1' })).toBe(false)
  })
