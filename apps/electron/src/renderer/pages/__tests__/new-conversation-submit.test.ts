...
let createFirstTurnSessionOptions: (typeof import("../NewConversationPage"))["createFirstTurnSessionOptions"];
...
beforeAll(async () => {
  ...
  mock.module("@/components/app-shell/input/InputContainer", () => ({
    InputContainer: () => null,
  }));
  mock.module(
    "@/components/extensions/extension-frontend-channel-store",
    () => ({
      getExtensionSessionBootstrap: () => undefined,
    }),
  );
  ({
...
describe("new conversation first-turn submission", () => {
  ...
  it("rejects incomplete or contradictory durability failure payloads", () => {
    const failure = (data: Record<string, unknown>) =>
    ...
    expect(
      parseUnpublishedPublicationFailure(
        failure({ retryable: true, terminal: false, outcome: "unpublished" }),
      ),
    ).toBeNull();
    expect(
      parseUnpublishedPublicationFailure(
        failure({ retryable: true, terminal: true, outcome: "published" }),
      ),
    ).toBeNull();
    expect(
      parseUnpublishedPublicationFailure(
        failure({ retryable: "yes", terminal: true, outcome: "unpublished" }),
      ),
    ).toBeNull();
    expect(
      parseUnpublishedPublicationFailure(
    ...
    expect(
      parseUnpublishedPublicationFailure(new Error("plain failure")),
    ).toBeNull();
  });
  ...
  it("attaches opaque Extension bootstrap state without persisting it in draft options", () => {
    const options: NewConversationDraftOptions = {
      provider: "provider-a",
      model: "model-a",
      thinkingLevel: "medium",
    ...
    const bootstrap = {
      "mortise-permissions": {
        "permission-mode": { mode: "ask" },
      },
    };

    expect(createFirstTurnSessionOptions(options, bootstrap)).toEqual({
      ...options,
      extensionBootstrap: bootstrap,
    });
    expect(options).not.toHaveProperty("extensionBootstrap");
  });
});
