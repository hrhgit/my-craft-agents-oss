...
describe('CLI extension interaction v1', () => {
  ...
  it('strictly recognizes versioned cancellation and settlement events', () => {
    const route = {
      requestId: 'request-1',
      extensionId: 'ask-user',
      runtimeId: 'runtime-1',
    ...
    expect(asExtensionInteractionTermination({
      ...route,
      type: 'extension_interaction_cancel',
      reason: 'timeout',
    })).toMatchObject({ type: 'extension_interaction_cancel', reason: 'timeout' })
    expect(asExtensionInteractionTermination({
      ...route,
      type: 'extension_interaction_settled',
      outcome: 'submitted',
    })).toMatchObject({ type: 'extension_interaction_settled', outcome: 'submitted' })
    expect(asExtensionInteractionTermination({
      ...route,
      type: 'extension_interaction_cancel',
  ...
  it('does not respond after the host has already cancelled the request', async () => {
    const controller = new AbortController()
    controller.abort()
    const calls: unknown[][] = []
    await handleExtensionInteractionInteractive(
      interactionEvent(),
      async (...args) => { calls.push(args) },
      undefined,
      controller.signal,
