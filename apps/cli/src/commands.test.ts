...
describe('parseArgs', () => {
  ...
  it('falls back to env vars for url and token', () => {
    ...
    const prevCa = process.env.MORTISE_TLS_CA

    process.env.MORTISE_SERVER_URL = 'ws://env-server:8080'
    process.env.MORTISE_SERVER_TOKEN = 'env-token'
    process.env.MORTISE_TLS_CA = '/env/ca.pem'
    ...
    try {
      ...
      if (prevUrl === undefined) delete process.env.MORTISE_SERVER_URL
      else process.env.MORTISE_SERVER_URL = prevUrl
      if (prevToken === undefined) delete process.env.MORTISE_SERVER_TOKEN
      else process.env.MORTISE_SERVER_TOKEN = prevToken
      if (prevCa === undefined) delete process.env.MORTISE_TLS_CA
      else process.env.MORTISE_TLS_CA = prevCa
    }
  })
...
describe('getValidateSteps', () => {
  ...
  it('includes session tool validation steps', () => {
    const names = getValidateSteps().map((s) => s.name)
    expect(names).toContain('session-tools:read_session')
  ...
  it('session tool steps come after tool use and before branching', () => {
    ...
    const sessionRead = names.indexOf('session-tools:read_session')
    const branch = names.indexOf('sessions:branch')
    expect(sessionRead).toBeGreaterThan(toolUse)
    expect(branch).toBeGreaterThan(sessionRead)
  })
