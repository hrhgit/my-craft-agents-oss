...
3. Read each selected `.agents/modules/<id>.md` document in full. If `frontend_impact.interaction_docs` is declared, read those linked interaction documents too. Use entry points, reusable capabilities, invariants, dependencies, consumers, frontend areas, interaction contracts, change-impact notes, and validation guidance to bound repository exploration.
